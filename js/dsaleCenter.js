...
function dis_next() {
  window.open("dwaiting_detail.html", "_self");
}

function dis_next_look() {
  window.open("dlook_detail.html", "_self");
}
...
function rep_next() {
  ...
  if ($("#dis_cost").text() == "" || $("#hour_cost").text() == "") {
    ...
    $(window.parent.parent.document).find("input[name='butie_cost']").val($("#butie_cost").html());
    $(window.parent.parent.document).find("input[name='checkTime']").val($("#repairday").val());
    
    window.open("drepaired_check.html", "_self");
  }
}
...
function to_rpt_400() {
  window.open("drepaireNeedDetail.html?reportID=" + base64encode($(window.document).find("input[name='reportId_400']").val()), "_self");
}
...
function to_rpt(event) {
  if (document.getElementById("status2").className == "status_ing") {
    ...
    //     '</div>'
    // );
    var cont = '<iframe src="dreport_detail.html" frameborder="0" scrolling="no" width="100%" height="1700"></iframe>';
    ...
    var cont = '<iframe src="dreport_detail.html" frameborder="0" scrolling="no" width="100%" height="2000px"></iframe>';
    $("#detail_frame").html(cont);
  }
}
...
function to_rpt_unpass(event) {
  ...
  // window.open("repaireNeedDetail.html?token="+$(window.parent.document).find("input[name='token']").val()+"&reportID=" + $(window.parent.document).find("input[name='reportId']").val(),"_self");
  var cont = '<iframe src="dreport_unpass.html" frameborder="0" scrolling="no" width="100%" height="2000px"></iframe>';
  $("#detail_frame").html(cont);
}
...
function to_dis(event) {
  ...
  var cont = '<iframe src="ddispatching_detail.html" frameborder="0" scrolling="no" width="100%" height="2100px"></iframe>';
  $("#detail_frame").html(cont);
}
...
function to_rep(event) {
  ...
  var cont = "<iframe id='ifm' src='drepaired_detail.html' frameborder='0' scrolling='no' width='100%' height='1800px'></iframe>"
  $("#detail_frame").html(cont);
...
function to_rep_unpass(event) {
  ...
  // var param = $(window.parent.document).find("input[name='token']").val()+","+$(window.parent.document).find("input[name='reportId']").val()+","+$(window.parent.document).find("input[name='repairId']").val();
  var cont = "<iframe id='ifm' src='drepair_unpass.html' frameborder='0' scrolling='no' width='100%' height='1800px'></iframe>"
  $("#detail_frame").html(cont);
...
function openUserVisit() {
  var cont = "<iframe id='ifm' src='duserVisit.html' frameborder='0' scrolling='no' width='100%' height='100%'></iframe>"
...
function to_complete(event) {
  ...
  // var param = $(window.parent.document).find("input[name='token']").val()+","+$(window.parent.document).find("input[name='reportId']").val()+","+$(window.parent.document).find("input[name='repairId']").val();
  var cont = "<iframe id='ifm' src='dcomplete.html' frameborder='0' scrolling='no' width='100%' height='1800px'></iframe>"
  $("#detail_frame").html(cont);
...
function to_close(event) {
  ...
  var cont = "<iframe id='ifm' src='dcloseDetail.html' frameborder='0' scrolling='no' width='100%' height='1300px'></iframe>"
  $("#detail_frame").html(cont);
}
    ...
    if (reason_describe.length <= 150) {
      ...
      if (reason_id == "0") {
        ...
        $.ajax({
          type: "post",
          async: false,
          url: ip_path + "/changfa_system/powerReport/PowerApproveReport.do",
          ...
          success: function (data) {
            if (data['head']['code'] == 200) {
              ...
              window.open("drepaireNeedDetail.html?token=" + base64encode(token) + "&reportID=" + base64encode(report_id), "_parent");
            } else {
...
function dispatching_to() {
  ...
  $.ajax({
    ...
    url: ip_path + "/changfa_system/powerDispatch/createPowerDispatch.do",
    data: {
      token: token,
      reportId: rpt_id,
      repairUserId: id,
    ...
    success: function (data) {
      if (data['head']['code'] == 200) {
        ...
        window.open("drepaireNeedDetail.html?token=" + base64encode(token) + "&reportID=" + base64encode(rpt_id), "_parent");
        //  var dispatchId = data['body']['result']['dispatchId'];
        //  $(window.parent.parent.document).find("input[name='reportId']").val(dispatchId);
...
function forHistoryDetail(event) {
  ...
  // $("#detail_frame").html(cont);
  window.open("drepaireNeedDetail.html?token=" + base64encode($(window.parent.parent.document).find("input[name='token']").val()) + "&reportID=" + base64encode(report_id), "_parent")
...
function forHistoryDetail_400(event) {
  ...
  // var cont = "<iframe id='ifm' src='repaired_detail.html' frameborder='0' scrolling='no' width='1724px' height='1800px'></iframe>"
  // $("#detail_frame").html(cont);
  window.open("drepaireNeedDetail.html?token=" + base64encode(token_400) + "&reportID=" + base64encode(report_id), "_self")
  ...
  if (reason_id == "") {
    ...
    if (reason_describe.length <= 150) {
      var json = {
        ...
        "examineSubsidyDays":$(window.parent.parent.document).find("input[name='checkTime']").val(),
        "examineSubsidyCost":$(window.parent.parent.document).find("input[name='butie_cost']").val()
      ...
      $.ajax({
        ...
        url: ip_path + "/changfa_system/powerRepair/approvePowerRepairNopass.do",
        contentType: 'application/json',
        data: JSON.stringify(json),
        success: function (data) {
          if (data['head']['code'] == 200) {
    ...
    if (reason_describe.length <= 150) {
      ...
      $.ajax({
        ...
        url: ip_path + "/changfa_system/powerRepair/approvePowerRepairClose.do",
        contentType: 'application/json',
        data: JSON.stringify(json),
        success: function (data) {
          if (data['head']['code'] == 200) {
...
function rep_check_pass() {
  ...
  $.ajax({
    ...
    url: ip_path + "/changfa_system/powerRepair/approvePowerRepairPass.do",
    data: {
      repairId: repairId,
    ...
    success: function (data) {
      ...
      if (data['head']['code'] == 200) {
        ...
        var report_id = $(window.parent.parent.document).find("input[id='hidden2']").val();
        window.open("drepaireNeedDetail.html?token=" + base64encode(token) + "&reportID=" + base64encode(report_id), "_parent");
        //$('#tianshu').html(data['body']['result']['examineSubsidyDays'])
      } else {
        alert(data['head']['message']);
      }
    }
...
function to_rptDetail() {
  if ($(window.document).find("input[name='reportId_400']").val() == undefined) {
    window.open("drepaireNeedDetail.html?reportID=" + base64encode($(window.parent.parent.document).find("input[name='reportId']").val()), "_parent");
  } else {
    window.open("drepaireNeedDetail.html?reportID=" + base64encode($("#rpt_400").val()), "_self");
  }
}
        ...
        if ($("input[name='repairType']:checked").val() == 2) {
          ...
          $.ajax({
            ...
            data: {
              ...
              matchBrandId: $('#consumptionType').find('option:selected').text() == "销售"?'':$("#pinpai").val(),
              saleId:$("#consumptionType").val(),
            },
            dataType: "json",
            success: function (data) {
              if (data['head']['code'] == 200) {
...
function to_lastReport() {
  ...
  window.open("drepaireNeedDetail.html?reportID=" + base64encode($(window.parent.parent.document).find("input[name='old_rptId']").val()), "_parent");
}

function to_lastReport_400() {
  $(window.parent.parent.document).find("input[name='history_400_mark']").val("");
  window.open("drepaireNeedDetail.html?reportID=" + base64encode($(window.parent.parent.document).find("input[name='old_rptId_400']").val()), "_parent");
}
...
function restartDispatch() {
  ...
  if (window.confirm(msg)) {
    ...
    // reDispatch = 1;
    window.open("ddispatching_checkRedis.html", "_self");
  }
}
      ...
      if (fee == 0) {
        if ($("#money").val() == "") {
          ...
          $.ajax({
            ...
            url: ip_path + "/changfa_system/powerReportComment/returnPowerVisit.do",
            data: {
              reportId: $(window.parent.parent.document).find("input[name='reportId']").val(),
              token: $(window.parent.parent.document).find("input[name='token']").val(),
...
function addVisitHistory() {
  ...
  $.ajax({
    ...
    url: ip_path + "/changfa_system/powerExamineFlow/powerExamineInsert.do",
    data: {
      reportId: $(window.parent.parent.document).find("input[name='reportId']").val(),
      teleNo: $("#seatNum").val(),
    ...
    success: function (data) {
      if (data['head']['code'] == 200) {
        ...
        $.ajax({
          type: "post",
          async: false,
          url: ip_path + "/changfa_system/powerExamineFlow/powerExamineSelectByDisId.do",
          data: {
            reportId: $(window.parent.parent.document).find("input[name='reportId']").val()
          },
          dataType: "json",
...
function makeCost() {
  ...
  if ($("#checkHour").val() == "" || $("#checkDistance").val() == "") {
    ...
    $.ajax({
      ...
      url: ip_path + "/changfa_system/powerRepair/countPowerCost.do",
      data: {
        ...
        //examineSubsidyDays:$("#repairday option:selected").val(),   
        examineSubsidyDays:$("#repairday").val(),
      ...
      success: function (data) {
        ...
        if (data['head']['code'] == 200) {
          ...
          $("#butie_cost").text(data['body']['result']['examineSubsidyCost']);
          //$("#checkTime").text(data['body']['result']['examineSubsidyDays']);
        } else {
          alert(data['head']['message']);
        }
      }
...
function editServicePrice(event) {
  ...
  $("#createHourPrice").val(event.parentNode.parentNode.childNodes[3].innerHTML);
  $("#createDistancePrice").val(event.parentNode.parentNode.childNodes[4].innerHTML);
}
...
//function createModelocg1(){
//    var pronum = $("body_td #number1").val();
///    var proname = $("body_td #createModelName1").val();
//    var tr=$("<tr></tr>");
//    //tr.html("<td>"+pronum+"</td><td>"+proname+"</td>"+"<td><a href='javascript:void(0)'>GET</a></td>");
//    tr.html("<td>"+<input type="checkbox" name="saleType" id='checkboxall' onclick="checkboxall()"/>+"</td><td>"+pronum+"</td>"+"<td>"+proname+"</td>");
//    $("#adminTbody").append(tr);
//}
//关闭弹框
function closeaddConfigtable(){
  $("#addConfigtable").hide();
}

function updateSeriesocg1() {
  $.ajax({
    type: "post",
    async: true,
    url: ip_path + "/changfa_system/powerPart/updatePowerFault.do",
    data: {
     id:$("#editid").val(),
    ...
    success: function (data) {
      ...
      $("input:checkbox:checked").each(function (index, elem) {
        $(this).parent().next().next().html($("#editSeriesName").val());
      });
      $('#editSeries').hide()
    }
  })
...
function daochu(){
  $.ajax({
    type: "post",
    async: true,
    //url: ip_path + "/changfa_system/powerPart/saleExcelDownload.do",
    dataType: "json",
    success: function () {
...
function delConfigtable(){
  ...
  if ($("input:checkbox:checked").length == 0) {
    ...
    $("input:checkbox:checked").each(function (index, elem) {
      if(editid == "") {
        ...
        editid = editid+","+$(this).attr("data-id");
        //editid = $(this).attr("data-id")+',';
      }
  ...
  $.ajax({
    type: "post",
    async: true,
    url: ip_path + "/changfa_system/powerPart/batchDelete.do",
    data: {
      ids:editid
     },
     dataType: "json",
     success: function (data) {
      //dgjnumber = $(obj).attr("data-number");//选择的编号
      if (data['head']['code'] == 200) {
          $.ajax({
              type: "post",
              url: ip_path + "/changfa_system/powerPart/selectByNameAndLineNum.do",
              data:{
                lineNum: dgjnumber,
                  //lineNum: $(".hqname").attr("data-id"),
                  pageSize:15,
                  pageNum:1,
              },
              dataType: "json",
              success: function (datas) {
                  if (datas['head']['code'] != 200) {
                      $("#adminTbody").empty();
                      $("#adminTbody").append(
                      ...
                      currentPage = datas['body']['result']['pageNum'];
                      totalPage = datas['body']['result']['pages'];
                      totalData = datas['body']['result']['total'];
                      $(".dqym").val(datas['body']['result']['pageNum']);
                      $("#adminTbody").empty();
                      for(var i=0;i< datas['body']['result']['data']['length']; i++ ){
                          if(datas['body']['result']['data']['length']>0){
                              $('#adminTbody').append(
                                  '<tr text-align="center" class="'+i+'">'+
                                  '<td height="33" id="ridao" class="th2" style="width:40px;">'+
                                  '<input type="checkbox" name="saleType" id="checkboxall" data-id="'+datas['body']['result']['data'][i]['id']+'" onclick="checkbox()"/>'+
                                  '</td>'+
                                  '<td id="bianhao" class="th2" height="33" style="text-align:left;text-indent:60px;">'+datas['body']['result']['data'][i]['partNo']+'</td>'+
                                  '<td id="mingcheng" class="th2" height="33"  style="text-align:left;text-indent:60px;">'+datas['body']['result']['data'][i]['partName']+'</td>'+
                                  '</tr>'
...
function editConfig() {
  ...
  if ($("input:checkbox:checked").length == 1) {
    $("input:checkbox:checked").each(function (index, elem) {
      event = elem;
      numberbh = $(this).parent().next().html();
      namemz = $(this).parent().next().next().html();
      editid = $(this).attr("data-id");
...
//alert(namemz);
  $("#editSeries").show();
  $("#editSeriesNum").val(numberbh);
  $("#editSeriesName").val(namemz);
  $("#editid").val(editid);
...
function addModelocg() {
  var name = $(".hqname").html();//选择的产品名
  if (name.length != 0) {
    $('#levelChoose').empty();
    //根据存储在元素里面的参数来判断是否是断层
    var level = $('.dgjclick').next().attr('title');
    var nextlevel = $('.dgjclick').next().html();
    var paramNum = ($('.dgjclick').children().html());
    if ((nextlevel - level) > 1) {
      $.ajax({
        type: "post",
        async: false,
        //url: ip_path + "/changfa_system/productCatalog/selectByParentNumAndLevel.do",
        url: ip_path + "/changfa_system/powerPart/addPowerFault.do",
        data: {
          level: nextlevel,
...
function createModel() {
  ...
  if (outerelem && outerelem.attr('class') == 'category-cont-pro') {
    ...
    if (!regNum.test($("#number").val())) {
      alert("产品编号格式错误！");
    } else if ($("#createModelName").val().length > 10) {
      alert("产品名称不超过10个字符！");
    } else {
      $.ajax({
        type: "post",
...
//  var proname = $('#createModelName').val();
//  $('#lineList').append(
//    '<li id="dgjli" class="dgjli">' +
//     '<span id="dgjclick" class="dgjclick" onclick="dgjclick(this)">' + proname + '</span>'+
//     '<span style="line-height:30px">-----</span>' + pronum +
//    '</li>'
//);
//pronum.empty();
//proname.empty();
...
var dgjname;
var dgjnumber;
function dgjclick(obj){
  
  $(obj).parent().css('color','#1ABC9C').siblings('color','#333');
  $(".dgjli").css("color",'#333');
  $(obj).parent().css('color','#1ABC9C');
  $(".dgjli").children("span").removeClass("hqname");
  $(obj).addClass("hqname");//对应选择的产品名
  $(obj).parent().css('color','#1ABC9C');
   dgjname = $(obj).attr("data-name");//选择的名字
   dgjnumber = $(obj).attr("data-number");//选择的编号
   $.ajax({
    type: "post",
    url: ip_path + "/changfa_system/powerPart/selectByNameAndLineNum.do",
    ...
    success: function (data) {
      if (data['head']['code'] != 200) {
        ...
        document.getElementById("barcon1").innerHTML = "";   
          ...
          totalPage = data['body']['result']['pages'];
          totalData = data['body']['result']['total'];
          $(".dqym").val(data['body']['result']['pageNum']);
          $("#adminTbody").empty();
        for(var i=0;i< data['body']['result']['data']['length']; i++ ){
          ...
          $('#adminTbody').append(
            '<tr text-align="center" class="'+i+'">'+
            '<tr text-align="center" class="'+i+'">'+
                 '<td height="33" id="ridao" class="th2">'+
                    '<input type="checkbox" name="saleType" id="checksingle" data-id="'+data['body']['result']['data'][i]['id']+'" onclick="checkbox()"/>'+
                  '</td>'+
                  '<td id="bianhao" class="th2" height="33" style="text-align:left;text-indent:60px;">'+data['body']['result']['data'][i]['partNo']+'</td>'+
                    '<td id="mingcheng" class="th2" height="33"  style="text-align:left;text-indent:60px;">'+data['body']['result']['data'][i]['partName']+'</td>'+
            '</tr>'
...
var pronum;
var proname;
function createModelocg2(){
   pronum = $('#number1').val();
   proname = $('#createModelName1').val();
  $.ajax({
    type: "post",
    url: ip_path + "/changfa_system/powerPart/addPowerFault.do",
    data:{
    ...
    if (!regNum.test($("#number").val())) {
      ...
      $.ajax({
        ...
        data: {
          ...
          name: $("#createModelName").val(),
          lineNum:$("#number").val(),
          number:$('.category-cont-bgdc').prev().prev().attr('title'),
...
function showrskCheckDetails(event) {
  ...
  //农机状态
  var statusnj = $(event).parent().prev().prev().prev().html();
  $(window.parent.document).find("input[name='statusnj']").val(statusnj);

//报警
  var baojing = $(event).parent().prev().prev().html();
  $(window.parent.document).find("input[name='baojing']").val(baojing);
...
function showrskCheckDetails1(event) {
  //报警
  var baojing = $(event).parent().prev().prev().html();
  $(window.parent.document).find("input[name='baojing']").val(baojing);
  ...
  var alarm = $(event).parent().prev().prev().html();
  $(window.parent.document).find("input[name='alarm']").val(alarm);
  window.open("riskcheck_detail1.html", "_self");
}
...
function dcancleDispatch() {
  if (window.confirm("确定要撤销该次派工吗？")) {
    if (window.confirm("撤销派工后请重新派工！")) {
      $.ajax({
        type: "post",
        async: false,
        url: ip_path + "/changfa_system/powerReport/powerReportNew.do",
        data: {
...
function closeDispatch() {
  if (window.confirm("确认强制关闭该派工单吗？")) {
    $.ajax({
      type: "post",
      async: false,
      url: ip_path + "/changfa_system/powerReport/focusClosePowerReport.do",
      data: {
        token: $(window.parent.parent.document).find("input[name='token']").val(),
        reportId: $(window.parent.parent.document).find("input[name='reportId']").val()
      },
...
//yemiantoogle();
  var chooseItem = "";
  var productNum = $("#productNum").val();//输入框
  var leixin = $("#product_num option:selected").val();
  if(leixin == 1) {
    chooseItem = "number";
  } else {
    chooseItem = "name";
          ...
          for(var i=0;i< data['body']['result']['data'].length; i++ ){
            ...
            $('#adminTbody').append(
              '<tr text-align="center">'+
                   '<td height="33" id="ridao" class="th2" style="width:40px;">'+
                      '<input type="checkbox" name="saleType" id="checkboxall" data-id="'+ data['body']['result']['data'][i]['id']+'" onclick="checkbox()"/>'+
                    '</td>'+
                    '<td id="bianhao" class="th2" height="33" style="text-align:left;text-indent:60px;">'+data['body']['result']['data'][i]['partNo']+'</td>'+
                    '<td id="mingcheng" class="th2" height="33"  style="text-align:left;text-indent:60px;">'+data['body']['result']['data'][i]['partName']+'</td>'+
              '</tr>'
...
function pageSplitsousuo() {
  ...
  document.getElementById("barcon1sousuo").innerHTML = tempStr;
}
function hideOrShowsousuo() {
  if (currentPage == 1) {
    ...
    for (var i = 0; i < $("#carList").find("tr").length; i++) {
      ...
      if (i % 2 == 0) {
        var arr = {
          ...
          "price": $("#carList").find("tr").eq(i).find("td").eq(6).html(),
  ...
  if (emptyCount == 0) {
    ...
    for (var i = 0; i < $("#carList").find("tr").length; i++) {
      if ($("#carList").find("tr").eq(i).find("td").eq(9).html() == 0 && $("#carList").find("tr").eq(i).find("td").eq(10).html() == 0) {
        if ($("#carList").find("tr").eq(i).find("td").eq(5).find("input").eq(0).val() > $("#carList").find("tr").eq(i).find("td").eq(11).html()) {
