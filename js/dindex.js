...
window.onload = function(){
    ...
    if(roleId == 1 || roleId == 2 || roleId == 21 || roleId == 23 || roleId == 17){
        ...
        $.ajax({
            type : "post",
            url : ip_path+"/changfa_system/machineFlow/powerCount.do",
            dataType : "json",
            success : function (data) {
                if(data['head']['code'] == 200){
                    $("#rptNumWaiting1").val(' '+data['body']['result']['reportCount']);
                }else{
                    alert(data['head']['message']);
                }
            }
...
function iframeInsert(event) {
    $(window.document).find("input[name='history_400_mark']").val('');
    $(window.document).find("input[name='longTextId']").val('');
    $(window.document).find("input[name='roleId_forLimit']").val('');
    var content = "<iframe id='ifm' src='" + event.parentNode.id + ".html' frameborder='0' scrolling='no' width='100%' height='4000px'></iframe>"
    $("#inner").html(content);
...
function newRpt1(){
    $(window.document).find("input[name='fastRptProcess']").val(1);
    var content = "<iframe id='ifm' src='dfastRepairNeed.html' frameborder='0' scrolling='no' width='100%' height='2500px'></iframe>"
    $("#inner").html(content);
    slideUp();
}
