(function($,window,document,undefined){ //用一个自调用匿名函数把插架代码包裹起来，防止代码污染
    $.fn.mySelect = function (options) {
       ...
       //生成option-item并追加展示
		...
		html += '<div class="select-picker-search-checked" >请选择产品线</div>';
		html += '<div class="select-picker-search-checked1" style="display:none"></div>';
 		html += '</div>';
 		html += '<div class="select-picker-options-wrp">';
 		html += '<div class="select-picker-options-serch">';
 		//html += '<input type="text" placeholder="">';去掉搜索框
 		html += '</div>';
 		...
 		$this.on('click',".select-picker-options-list-item",function(){ 
			...
			$this.find(".select-picker-options-list-item").each(function(){ 
				...
				if(_this.find('.duihao-checked').length > 0){
					//alert( $.trim(_this.attr('data-id')) )
					//alert( _this.html() )
					//alert( _this.find('span').attr('data-id') )
					//checkedArr.push($.trim(_this.find('span').attr('data-id')))
					checkedArr.push($.trim(_this.text()))
					checkedArr1.push($.trim(_this.find('span').attr('data-id')))
				}
			})
			if (checkedArr.length > 0) {
			    ...
			    $this.find('.select-picker-search-checked').text('请选择产品线').css('color', '#757575');
			}
			if (checkedArr1.length > 0) {
				$this.find('.select-picker-search-checked1').text(checkedArr1.join(','));
			    // $this.find('.select-picker-search-checked').text(checkedArr.join(',')).css('color', '#fff');
			} else {
			    $this.find('.select-picker-search-checked1').text('请选择产品线').css('color', '#757575');
			}
 		})
 		// 前端实现下拉搜索 
